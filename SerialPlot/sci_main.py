...
class Sci_UiCtl(sci_tool.Ui_MainWindow):
    def __init__(self,MainWindow):
        ...
        self.sciopenButton.clicked.connect(self.SciOpenButton_Click)
        self.clrcontentbutton.clicked.connect(self.ClrButtonProcess)
        self.mainsend_Button.clicked.connect(self.MainSendButtonProcess)
        self.sendclr_Button.clicked.connect(self.ClrSendButtonProcess)
        self.clrcntbutton.clicked.connect(self.ClrCntButtonProcess )
        self.cmd1sned_Button.clicked.connect(self.Cmd1SendButtonProcess)
        self.cmd2sned_Button.clicked.connect(self.Cmd2SendButtonProcess)
        self.cmd3sned_Button.clicked.connect(self.Cmd3SendButtonProcess)
        self.cmd4sned_Button.clicked.connect(self.Cmd4SendButtonProcess)
        self.cmd5sned_Button.clicked.connect(self.Cmd5SendButtonProcess)
        self.savecontentbutton.clicked.connect(self.SaveRecButtonProcess)
        self.x1save_button.clicked.connect(self.X1SaveButtonProcess)
        self.x1clr_button.clicked.connect(self.X1ClrButtonProcess)
        self.x2save_button.clicked.connect(self.X2SaveButtonProcess)
        self.x2clr_button.clicked.connect(self.X2ClrButtonProcess)
        self.x3save_button.clicked.connect(self.X3SaveButtonProcess)
        self.x3clr_button.clicked.connect(self.X3ClrButtonProcess)
    ...
    def SciOpenButton_Click(self):
         ...
         if clickstatus:
            #得到串口的设置参数
            comread = 'COM'+str(self.portcomtext.text())
            print(comread)
            ...
            try:
                ...
                QtWidgets.QMessageBox.warning(None, '端口警告',"端口无效或者不存在", QtWidgets.QMessageBox.Ok)
        ...
        if self.x1_checkBox.isChecked() == True:
            try:
                ...
                if self.x1_low > self.x1_high:
                     QtWidgets.QMessageBox.warning(None, '错误',"X1填写数据出错", QtWidgets.QMessageBox.Ok)
        ...
        if self.x2_checkBox.isChecked() == True:
            try:
                ...
                if self.x2_low > self.x2_high:
                     QtWidgets.QMessageBox.warning(None, '错误',"X2填写数据出错", QtWidgets.QMessageBox.Ok)
        ...
        if self.x3_checkBox.isChecked() == True:
            try:
                ...
                if self.x3_low > self.x3_high:
                     QtWidgets.QMessageBox.warning(None, '错误',"X3填写数据出错", QtWidgets.QMessageBox.Ok)
    ...
    def SerialSend(self,sdata):
        try:
            self.senddatacnt += self._serial.write(sdata)
             ...
             QtWidgets.QMessageBox.warning(None, 'Error',"数据格式错误", QtWidgets.QMessageBox.Ok)
    ...
    def SaveRecButtonProcess(self):
        filename = QtWidgets.QFileDialog.getSaveFileName(self.savecontentbutton, 'Save File', '.',"Text file(*.txt);;All file(*.*)")
    ...
    def X1SaveButtonProcess(self):
        filename = QtWidgets.QFileDialog.getSaveFileName(self.x1save_button, 'Save File', '.',"Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
    ...
    def X2SaveButtonProcess(self):
        filename = QtWidgets.QFileDialog.getSaveFileName(self.x2save_button, 'Save File', '.',"Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
    ...
    def X3SaveButtonProcess(self):
        filename = QtWidgets.QFileDialog.getSaveFileName(self.x3save_button, 'Save File', '.',"Text file(*.txt);;All file(*.*)")
        fname = open(filename, 'w')
